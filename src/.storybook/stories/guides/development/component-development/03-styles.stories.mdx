...
The styles for Outline components are kept in a typical `.css` file such as `nub-widget.css` that lives parallel to the
component `nub-widget.ts`. These component CSS files utilize [PostCSS](https://postcss.org/) processing and a variety of plugins to handle various
...
styles from our `nub-widget.css.lit.ts` file. Any CSS that is included should be wrapped in Lit’s `css` template literal.
This is where Outline applies additional logic to handle taking a standard CSS file like `nub-widget.css`,
and converts to `nub-widget.css.lit.ts` which is then imported into the main component file, `nub-widget.ts` as seen in the following code sample.
...
> The following example shows both importing content from the default `nub-widget.css.lit.ts`
...
> The same effect can be accomplished in the `nub-widgets.css` file by conditionally
...
Below is an example of Tailwind classes from our `nub-link.css` file.
...
<Story name="Using Tailwind CSS">
  {() => {
    return html`
<nub-code-block language="css" class="mb-6">
:host,
a,
::slotted(a) {
...
Below is an example of CSS Custom Property usage from our `nub-link.css` file.
In this instance, we have utilized 3 custom CSS variables to help us alter the color of a link when using the `nub-link` component.
...
:root {
   ...
   */
  --nub-link-color-default: var(--blue-responsible-darken-1);
  --nub-link-color-hover: var(--blue-responsible-lighten-1);
  --nub-link-color-focus: var(--blue-responsible-lighten-1);
  ...
}
</nub-code-block>
`;
...
as well as the configuration for the `nub-link` component.
...
Let's take a look at a sample of this in action. We will use the `nub-link` component again.
...
::slotted(a) {
  @apply no-underline font-body transition-colors duration-300;
  color: var(--nub-link-color-default);
}
:host(:hover),
a:hover,
::slotted(a:hover) {
  @apply underline;
  color: var(--nub-link-color-hover) !important;
}
:host(:focus),
a:focus,
::slotted(a:focus) {
  @apply underline nub-none;
  color: var(--nub-link-color-focus) !important;
}
</nub-code-block>
    `;
  }}
...
<Story name="-- nub-link.css.lit.ts">
  {() => {
    return html`
<nub-code-block language="jsx" class="mb-6">
import { css } from 'lit';
export default css\`
...
The above `.ts` file is now what we include directly to our `nub-link.ts` component,
