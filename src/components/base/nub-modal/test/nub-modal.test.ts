...
describe(componentElementName, () => {
  ...
  it('Slot elements while interacting with the modal.', async () => {
    ...
    /**
     * <button slot="nub-modal--trigger">
     *  Open small modal.
     * </button>
     */
    const triggerElement = document.createElement('button');
    triggerElement.setAttribute('slot', 'nub-modal--trigger');
    triggerElement.innerText = 'Open small modal';
    modalElement.append(triggerElement);

    /**
     * <h2 slot="nub-modal--header">Modal header text</h2>
     */
    const headerElement = document.createElement('h2');
    headerElement.setAttribute('slot', 'nub-modal--header');
