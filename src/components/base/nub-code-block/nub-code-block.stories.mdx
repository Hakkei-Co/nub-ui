...
<Story name="TypeScript Block">
  {() => {
    return html`
      <div class='w-1/2'>
      <nub-code-block language="typescript">
          const helloWorld = 'Hi'
          const function = () => {
...
Wrap your code/mark-up in the `nub-code-block` component and pass in the desired language to the `lang` attribute
